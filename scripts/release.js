...
function run(cmd, msg) {
  ...
  try {
    ...
    console.warn(`⚠️  Commande échouée : ${cmd}`);
  }
}
...
try {
  ...
  // 2️⃣ Sauvegarde du travail
  run("git add .", "Ajout des fichiers modifiés");
  run('git stash push -m "Préparation release auto" || echo "Rien à stasher"', "Sauvegarde temporaire du travail");
  ...
  run("git stash pop || echo 'Pas de stash à restaurer'", "Restauration du stash dans codex-dev");

  // 4️⃣ Commit et push sur codex-dev
  run("git add .", "Ajout des fichiers à commiter");
  run('git commit -m "chore: release auto - update MathALEA & build config" || echo "Aucun changement à valider"', "Commit sur codex-dev");
  ...
  run("git merge codex-dev --no-edit || echo 'Conflits détectés, à résoudre manuellement'", "Fusion de codex-dev dans main");
